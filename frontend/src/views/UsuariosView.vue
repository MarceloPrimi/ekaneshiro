<template>
  <div>
        ...
        <div v-for="u in usuarios" :key="u.id" class="p-4">
          <div class="flex items-start justify-between gap-2 mb-1">
            <div class="min-w-0">
              ...
              <p class="text-xs text-gray-400 truncate mt-0.5">{{ u.username }}</p>
            </div>
          ...
          <div class="flex gap-2 mt-3">
            ...
            <button @click="abrirModalSenhaAdmin(u)" class="flex-1 text-xs font-medium text-orange-600 bg-orange-50 hover:bg-orange-100 rounded-lg py-2 px-3">Senha</button>
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="u in usuarios" :key="u.id" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3 text-right space-x-2">
              ...
              <button @click="abrirModalSenhaAdmin(u)" class="text-xs text-orange-600 hover:underline font-medium">Senha</button>
              <button
      ...
      <div class="bg-white w-full sm:max-w-sm sm:rounded-xl rounded-t-3xl shadow-xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div class="flex-1 overflow-y-auto px-6 pt-5 pb-2">
          ...
          <form id="form-editar-usuario" @submit.prevent="salvarEdicao" class="space-y-4">
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Usuário (login)</label>
              <input v-model="formEditar.username" minlength="3"
                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-rose-400"
                placeholder="sem espaços ou @" />
            </div>
            <div>
    ...
    <div v-if="modalSenhaAdmin" class="fixed inset-0 bg-black/40 flex items-end sm:items-center justify-center z-50 p-4" @click.self="modalSenhaAdmin = false">
      <div class="bg-white w-full sm:max-w-sm sm:rounded-xl rounded-t-3xl shadow-xl max-h-[90vh] flex flex-col overflow-hidden">
        <div class="sm:hidden w-10 h-1 bg-gray-300 rounded-full mx-auto mt-3 mb-1 flex-shrink-0"></div>
        <div class="flex-1 overflow-y-auto px-6 pt-5 pb-2">
          <h3 class="text-lg font-semibold text-gray-800 mb-1">Redefinir Senha</h3>
          <p class="text-xs text-gray-400 mb-4">{{ usuarioSenhaAdmin?.nome }}</p>
          <form id="form-senha-admin" @submit.prevent="salvarSenhaAdmin" class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Nova Senha *</label>
              <input v-model="formSenhaAdmin.nova_senha" type="password" required minlength="6" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-rose-400" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Confirmar Senha *</label>
              <input v-model="formSenhaAdmin.confirmar" type="password" required minlength="6" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-rose-400" />
            </div>
            <p v-if="erroSenhaAdmin" class="text-sm text-red-600">{{ erroSenhaAdmin }}</p>
          </form>
        </div>
        <div class="flex gap-2 px-6 py-4 border-t border-gray-100 flex-shrink-0">
          <button type="button" @click="modalSenhaAdmin = false" class="flex-1 border border-gray-200 text-gray-600 rounded-lg py-2.5 text-sm hover:bg-gray-50">Cancelar</button>
          <button type="submit" form="form-senha-admin" :disabled="salvando" class="flex-1 bg-orange-600 hover:bg-orange-700 text-white text-sm font-medium rounded-lg py-2.5 disabled:opacity-60">
...
const modalSenhaAdmin = ref(false)
const usuarioParaExcluir = ref(null)
const usuarioSenhaAdmin = ref(null)
const editandoId = ref(null)

const erroCriar = ref('')
const erroEditar = ref('')
const erroSenhaAdmin = ref('')

const formCriar = ref({ nome: '', username: '', password: '', role: 'recepcionista' })
const formEditar = ref({ nome: '', username: '', role: 'recepcionista', ativo: true })
const formSenhaAdmin = ref({ nova_senha: '', confirmar: '' })
...
function abrirModalCriar() {
  formCriar.value = { nome: '', username: '', password: '', role: 'recepcionista' }
  erroCriar.value = ''
...
function abrirModalEditar(u) {
  ...
  formEditar.value = { nome: u.nome, username: u.username, role: u.role, ativo: u.ativo }
  erroEditar.value = ''
  modalEditar.value = true
}

function abrirModalSenhaAdmin(u) {
  usuarioSenhaAdmin.value = u
  formSenhaAdmin.value = { nova_senha: '', confirmar: '' }
  erroSenhaAdmin.value = ''
  modalSenhaAdmin.value = true
}
...
async function salvarSenhaAdmin() {
  erroSenhaAdmin.value = ''
  if (formSenhaAdmin.value.nova_senha !== formSenhaAdmin.value.confirmar) {
    erroSenhaAdmin.value = 'As senhas não conferem.'
    return
  }
  salvando.value = true
  try {
    await api.patch(`/usuarios/${usuarioSenhaAdmin.value.id}/senha`, {
      nova_senha: formSenhaAdmin.value.nova_senha,
    })
    modalSenhaAdmin.value = false
    toastSucesso('Senha redefinida com sucesso!')
  } catch (e) {
    erroSenhaAdmin.value = e.response?.data?.detail || 'Erro ao redefinir senha.'
  } finally {
    salvando.value = false
  }
}
